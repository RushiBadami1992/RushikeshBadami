...
;;GOAL:To implement a screensaver using higher order functions.To use generalized functions
;;where ever possible
...
;;--(cons Point LOP)
;;INTERPRETATION:
;;ListofPoints is a list of posns 
;;Point is a Posn
;;lop-fn:LOP->??
...
;; EXAMPLE:Refer to test for example.
...
;;  DESIGN-STRATERGY:Use HOF maps on rects.
...
;;DESIGN STRATERGY:Use cases of collision on rect
...
;;EXAMPLE:Check test for example.
;;DESIGN STRATERGY:Use cases of corner-collision on rect
...
;; DESIGN-STRATERGY:Use HOF map on rects     
...
(define (rect-after-key-event rect kev)
  (cond
    [(key=? LEFT kev) (change-velocity-along-x-axis rect -)]
    [(key=? RIGHT kev)(change-velocity-along-x-axis rect +)]
    [(key=? UP kev)(change-velocity-along-y-axis rect -)]
    [(key=? DOWN kev)(change-velocity-along-y-axis rect +)]
    [(key=? REMOVEPEN kev)(add-or-remove-pen-from-rectangle rect false)]
    [(key=? PENDOWN kev)(add-or-remove-pen-from-rectangle rect  true)]
    [else rect]))
;;TEST
(begin-for-test
  ...
  (check-equal? (rect-after-key-event RECT-REMOVE-PEN PENDOWN)RECT-PENDOWN
                "Should add  pen"))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; add-or-remove-pen-from-rectangle:Rectangle Boolean->Rectangle
;; GIVEN:A rectangle
;; RETURNS:A rectangle with  pen dropped or removed from it.
;; EXAMPLE:Check test of rect-after-key-event function for example
;; DESIGN-STRATERGY:Use template for Rectangle on rect.
(define (add-or-remove-pen-from-rectangle r pen)
  (if(rect-selected? r)
     ...
     r))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; change-velocity-along-x-axis:Rectangle(Int NonNegInt->Int) ->Rectangle
...
(define (change-velocity-along-x-axis r operator)
  (if(rect-selected? r)
     (make-rect (rect-x r)(rect-y r)
                (operator (rect-vx r) INCREASE-VELOCITY)(rect-vy r)
                (rect-selected? r)(rect-mousex r)
                (rect-mousey r)(rect-pen-down? r)
                (rect-pen-drop-posns r))
     r))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; change-velocity-along-y-axis:Rectangle(Int NonNegInt->Int)->Rectangle
...
(define (change-velocity-along-y-axis r operator)
  (if(rect-selected? r)
     (make-rect (rect-x r)(rect-y r)
                (rect-vx r)(operator (rect-vy r)INCREASE-VELOCITY)
                (rect-selected? r)(rect-mousex r)
                (rect-mousey r)(rect-pen-down? r)
                (rect-pen-drop-posns r))
     r))
...
(define (rects-after-mouse-event rects mx my mev)
  (map
   ;;Rectangle->Rectangle
   (lambda (rect) (rect-after-mouse-event rect mx my mev))
   rects))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;RETURNS:A list of rectangles with there trace painted on empty scene.
;;EXAMPLE:Check test for example
;;DESIGN-STRATERGY:Use HOF folder on lor
(define (list-of-rectangles lor s)
  (foldr
   ;;Rectangle Scene -> Scene
   (lambda (x r) (place-rectangle x r))
   EMPTY-CANVAS
   lor))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; EXAMPLE:Check test for example
;; DESIGN-STRATERGY:Use HOF foldr  on lst

(define (draw-trace lst s)
  (foldr
   ;;Posn Scene->Scene
   (lambda (x s)(place-dot x s))
   s
   lst))
