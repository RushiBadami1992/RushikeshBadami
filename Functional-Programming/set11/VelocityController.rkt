...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; a VelocityController is a (new VelocityController% [model Model<%>][x Int][y Int]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  ;; DESIGN-STRATEGY:Combine Simpler functions  
    (define (real-width) (max width (+ (image-width (data-image)) 10)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;    
;; real-height:-> Int
;; RETURNS:A integer which is the height of image
;; EXAMPLE:Check test for example
;; DESIGN-STRATEGY:Combine Simpler functions    
    (define (real-height) (max height (+ (image-height (data-image)) 10)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    (define/override (after-key-event kev)
      (if text-selected?
        (local((define my-p (send model get-p)))
            ...
            [(key=? LEFT kev)
             (send model execute-command
               (make-particle (particle-x my-p) (particle-y my-p)
                              (- (particle-vx my-p) INCREASE-VELOCITY)
                              (particle-vy my-p)))]
            [(key=? RIGHT kev)
             (send model execute-command
               (make-particle (particle-x my-p) (particle-y my-p)
            ...
            [(key=? UP kev)
             (send model execute-command
               (make-particle (particle-x my-p) (particle-y my-p) (particle-vx my-p) 
                              (- (particle-vy my-p) INCREASE-VELOCITY)))]
            [(key=? DOWN kev)
             (send model execute-command
               (make-particle (particle-x my-p) (particle-y my-p) (particle-vx my-p) 
                              (+ (particle-vy my-p) INCREASE-VELOCITY)))]
...
;; EXAMPLE:Check test for example
;; DESIGN-STRATEGY:Cases on if text is selected or not    
...
;; DESIGN-STRATEGY:Combine Simpler Functions    
    ...
    (define (data-image)
      (local((define my-p (send model get-p)))
      (above
        (text "  Arrow keys change velocity" 8 (current-color text-selected?))
        (text (string-append
                "X = "
...
;; GIVEN:X,Y coordinate  or VX,VY of particle
...
(begin-for-test
  (local((define m (new Model%))
         (define p-ctrl (new VelocityController% [model m])))
  (check-equal?
  ...
  (check-equal?
    (particle-vx (dynamic-get-field 'p m))
    0)
  (check-equal?
    (particle-vy (dynamic-get-field 'p m))
    -5)
