#lang racket
;; Used to implement the XY controller in MVC design
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; a XYController% is a (new XYController% [model Model<%>][x Int][y Int]
...
(define XYController%
  ...
  (class* Controller% ()
    ...
    (field [ground (rectangle MAX-WIDTH MAX-HEIGHT "outline" "blue")])
       ...
       (begin
         (send model execute-command 
           (make-particle (+ (particle-x my-p) (- mx saved-mx))
                          (+ (particle-y my-p) (- my saved-my))
                          (particle-vx my-p) (particle-vy my-p)))
         (set! saved-mx mx)
         (set! saved-my my))
        void))))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(begin-for-test
  (local((define m (new Model%))
         (define p-ctrl (new XYController% [model m])))
  (check-equal?
  ...
  (send p-ctrl after-button-down 60 35); select the controller
  ...
  (send p-ctrl render-my-image)
  (send p-ctrl after-drag 140 110); move by 10 along y axis -10 along x axis
