#lang racket
;;Implements the YController in MVC design 
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;  YController% is a (new YController% [model Model<%>][x Int][y Int]
...
(define YController%
  ...
  (class* Controller% ()
    ...
    (field [ground (rectangle OFFSET MAX-HEIGHT "outline" "blue")])
    ;; defines the outer radius of particle
    (field [radius-out 10])
    ;; defines the inner radius of particle
    (field [radius-in 4])
    ;defines a particle
    (field [ball (place-image (circle radius-in "solid" "black") 10 10
...
;; DESIGN-STRATEGY:Combine Simpler Functions     
    (define/override (controller-selected? other-x other-y)
      (local(
             (define rectangle-width (image-width ground))
             (define rectangle-height (+ (image-height ground) OFFSET))
    ...
    (define/override (after-button-down mx my)
      (local(
            ...
            (define (is-ball-selected?)
              (and
                (<= (abs (- mx x)) (/ OFFSET HALF))
                (<= (abs (- my y)) (/ MAX-HEIGHT HALF)))))
      (begin 
      ...
      (if ball-selected?
       (begin
         (send model execute-command 
           (make-particle (particle-x my-p) (+ (particle-y my-p)
                                               (- my saved-my)) (particle-vx my-p)
                                                                (particle-vy my-p)))
         (set! saved-mx mx)
         (set! saved-my my))
        void))))
...
(begin-for-test
  (local((define m (new Model%))
         (define p-ctrl (new YController% [model m])))
  (check-equal?
  ...
  (send p-ctrl after-button-down 135 35); select the controller
  ...
  (send p-ctrl render-my-image)
  (send p-ctrl after-drag 150 110); move by 10 along y axis
  (check-equal? (particle-y (send m get-p)) 60)
))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
