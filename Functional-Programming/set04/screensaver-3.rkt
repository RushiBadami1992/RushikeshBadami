...
;;   (rect-selected? r)(rect-mousex r)(rect-mousey r)))
...
(define RECT-AT-CENTER
  (make-rect CENTER-X CENTER-Y 0 0 false 0 0))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;TEST CONSTANTS
(define MOUSEX 35)
(define MOUSEY 125)
...
(define RECT1-AFTER-TICK
  (make-rect (+ RECT1-X VX1) (+ RECT1-Y VY1) VX1 VY1 false 0 0))
...
(define RECT-VELOCITY-LEFT
  (make-rect CENTER-X CENTER-Y (- 2) 0 true 0 0))
(define RECT-VELOCITY-RIGHT
  (make-rect CENTER-X CENTER-Y  2 0 true 0 0))
(define RECT-VELOCITY-UP
  (make-rect CENTER-X CENTER-Y 0 (- 2) true 0 0))
(define RECT-VELOCITY-DOWN
  (make-rect CENTER-X CENTER-Y 0  2  true 0 0))
(define RECT-CENTER-SELECTED
  (make-rect CENTER-X CENTER-Y 0 0 true 0 0))
...
;;  EXAMPLE:Check test of world after tick for  example
...
;;DESIGN STRATERGY:Use template of Rectangle on rect
...
;;RETURNS:A boolean value which returns true if given rectangle collides with
;;corner
;;EXAMPLE:Check Test for example
;;DESIGN STRATERGY:Combine Simpler Function
...
;;RETURNS:A boolean value which returns true if it colliding with one of the
;;walls
;;EXAMPLE:Check test for example 
...
(begin-for-test
  (check-equal?
   (change-velocities-after-corner-hit RECT-AT-LT-CORNER) RECT-AF-LT-CORNER
                "The velocities should reverse after Left top corner hit")
  (check-equal?
   (change-velocities-after-corner-hit RECT-AT-RT-CORNER) RECT-AF-RT-CORNER
                "The velocities should reverse after right top corner hit")
  (check-equal?
   (change-velocities-after-corner-hit RECT-AT-LB-CORNER) RECT-AF-LB-CORNER
                "The velocities should reverse after right top corner hit")
  (check-equal?
   (change-velocities-after-corner-hit RECT-AT-RB-CORNER) RECT-AF-RB-CORNER
...
(begin-for-test
  (check-equal?
   (change-velocity-after-boundary-hit RECT-AT-LW-BOUNDRY) RECT-AF-LW-BOUNDRY
                "The velocity should reverse after bottom hit")
  (check-equal?
   (change-velocity-after-boundary-hit RECT-AT-RB-BOUNDRY) RECT-AF-RB-BOUNDRY
                "The velocity should reverse after right boundary hit")
  (check-equal?
   (change-velocity-after-boundary-hit RECT-AT-UB-BOUNDRY) RECT-AF-UB-BOUNDRY
                "The velocity should reverse after upper boundary hit")
  (check-equal?
   (change-velocity-after-boundary-hit  RECT-AT-LB-BOUNDRY)RECT-AF-LB-BOUNDRY
...
;;RETURNS:A boolean value as true if rectangle collides in the rightbottom
;;corner
;;EXAMPLE:Check the test of check-for-corner-collision?
;;DESIGN-STRATERGY:Use template for Rectangle on rect
...
;;RETURNS:A rectangle after moving by the given velocities in a single clock
;;tick
;;DESIGN STRATERGY:Use template for Rectangle on rect
...
;; DESIGN-STRATERGY:Use template for Rectangle on r
...
;;DESIGN-STRATERGY:Use template for LOR on rect
...
(begin-for-test
  (check-equal?
   (rect-after-button-up RECT-FOR-BUTTONDOWN MOUSEX MOUSEY)RECT-FOR-BUTTONUP
                "Should unselect the mouse on buttondown")
   (check-equal?
    (rect-after-button-up RECT-FOR-BUTTONUP MOUSEX MOUSEY)RECT-FOR-BUTTONUP
                "Should unselect the mouse on buttondown"))
...
;;list-of-rectangles:LOR Scene->Scene
;;GIVEN:A list of rectangle and empty scene
;;RETURNS:A list of rectangles painted on empty scene.
;;EXAMPLE:Check test for example
;;DESIGN-STRATERGY:Use template for LOR on rects
(define (list-of-rectangles rects s)
  (cond
    [(empty? rects) EMPTY-CANVAS]
    [else  (place-rectangle (first rects) (list-of-rectangles(rest rects) s))]))
